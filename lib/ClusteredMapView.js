...
const ClusteredMapView = ({
  ...
  useEffect(() => {
    ...
    React.Children.forEach(children, (child, i) => {
      if (isMarker(child)) {
        rawData.push(markerToGeoJSONFeature(child, i, index2++));
      } else {
  ...
  const _onRegionChangeComplete = region => {
    if (superCluster) {
      ...
      if (zoom >= 16 && markers.length === 1 && clusterChildren) {
    ...
    >
      {markers.map(marker =>
        marker.properties.point_count === 0 ? (
          propsChildren[marker.properties.index2]
        ) : !isSpiderfier ? (
          renderCluster ? (
            renderCluster({
      ...
      {spiderMarkers.map((marker) => {
        return (
          propsChildren[marker.index2]
            ? React.cloneElement(propsChildren[marker.index2], { coordinate: { ...marker } })
            : null
        )
      })}
