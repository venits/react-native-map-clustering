...
const ClusteredMapView = forwardRef(
  (
    ...
    //new state to store markers you dont want to cluster
    ...
    function getStringFromReactFormat(reactKey) {
      ...
      // Return the substring after "$" or the whole string if "$" is not present
      ...
      propsChildren.forEach((child, index) => {
        if (isMarker(child)) {
          if (notToCluster.includes(getStringFromReactFormat(child.key))) {
            // check if key is in notToCluster prop and if so add to a temp array
